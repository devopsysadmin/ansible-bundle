...
class Bundle(object):
	...
	def download(self):
		...
		if shell.isdir(self.path):
			...
			func = git.get
			args = self.version
		shell.echo('%s %s from %s (%s)...' %(msg, self.name, self.url, self.version), False)
		ok = func(args) if args else func()
		if ok:
			shell.echo(shell.bcolors.OKGREEN + 'OK' + shell.bcolors.ENDC)
		else:
			shell.echo(shell.bcolors.FAIL + 'ERROR' + shell.bcolors.ENDC)
		return ok
